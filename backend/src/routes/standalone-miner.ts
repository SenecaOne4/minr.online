...
router.get('/', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!profile || profileError) {
      console.log('[standalone-miner] Profile not found, creating with upsert for user:', userId);
      ...
      if (createError) {
        console.error('[standalone-miner] Error creating profile:', createError);
      ...
      if (!newProfile) {
        console.error('[standalone-miner] Profile upsert returned no data');
