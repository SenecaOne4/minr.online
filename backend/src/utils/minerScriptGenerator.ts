...
/**
 * Generate Python miner script with embedded configuration
 */
export function generateMinerScript(config: MinerConfig): string {
  // Read template file
  const templatePath = path.join(__dirname, '../../miner-scripts/miner-template.py');
  ...
  return template
    .replace(/{{USER_EMAIL}}/g, config.userEmail)
    .replace(/{{BTC_WALLET}}/g, config.btcWallet)
    .replace(/{{STRATUM_ENDPOINT}}/g, config.stratumEndpoint)
    .replace(/{{API_URL}}/g, config.apiUrl)
    .replace(/{{WORKER_NAME}}/g, config.workerName || `minr.${config.userEmail.split('@')[0]}`);
}
